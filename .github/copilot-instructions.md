...
**Plugin GUID**: `com.rikal.megashot`
...
| File | Purpose |
|---|---|
| `MegaShot/Class1.cs` | Main plugin entry, all BepInEx config definitions, FileSystemWatcher for live config reload, manual GetDamage patch |
| `MegaShot/CrossbowPatches.cs` | All Harmony patches, firing logic, HUD, zoom, sound, animation, building damage, object destruction, DoT, bolt stacks |
| `MegaShot/MegaShotItem.cs` | Custom MegaShot crossbow item � clone, registration, per-level damage, per-level recipe |
| `MegaShot/MegaShot.csproj` | Project file with all assembly references |
...
- Always check `MegaShotPlugin.ModEnabled.Value` before applying any patch logic
...
- All Fire properties use **game defaults** from the prefab (no config overrides)
...
All configs are in BepInEx config file: `com.rikal.megashot.cfg`
...
| Key | Type | Default | Range | Description |
|---|---|---|---|---|
| `Enabled` | bool | `false` | � | Write ALT-fire hit diagnostics to `Desktop\MegaShot_Diagnostic.txt` (prefab names, component types, HP, tier) |
...
| `..\Latest Release\MegaShot\MegaShot.dll` | Release-staged DLL (uploaded to GitHub; MegaLoad's updater delivers to the profile) |
| `...\BepInEx\config\com.rikal.megashot.cfg` | Config file |
...
- Build stages the DLL in `..\Latest Release\MegaShot\MegaShot.dll` — it is **not** copied into any BepInEx profile folder.
- After running the script: `gh release create vX.Y.Z "..\Latest Release\MegaShot\MegaShot.dll" --repo ccmrik/MegaShot`, then bump `MegaLoad/mod-manifest.json` and re-upload it to the MegaLoad release. MegaLoad's auto-updater delivers to the active profile from there.
